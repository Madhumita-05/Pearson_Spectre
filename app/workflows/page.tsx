import { Sidebar } from "@/components/sidebar"
import { TopBar } from "@/components/top-bar"
import { GlassCard, Badge, Button } from "@/components/ui-components"
import Link from "next/link"

const workflows = [
  {
    id: "spec-20240614-a7f3",
    contract: "Razorpay_Vendor_Agreement_Q2_2024.pdf",
    size: "2.4 MB · 28 pages",
    severity: "violation" as const,
    status: "complete" as const,
    risk: "7.2",
    updated: "2m ago",
  },
  {
    id: "spec-20240613-b2c1",
    contract: "PhonePe_NDA_Draft_v3.pdf",
    size: "1.2 MB · 14 pages",
    severity: "high" as const,
    status: "running" as const,
    risk: "—",
    updated: "5m ago",
  },
  {
    id: "spec-20240612-d4e5",
    contract: "Paytm_Data_Processing_Agreement.pdf",
    size: "3.1 MB · 42 pages",
    severity: "medium" as const,
    status: "complete" as const,
    risk: "5.8",
    updated: "1h ago",
  },
  {
    id: "spec-20240611-f6g7",
    contract: "HDFC_Cloud_Services_SLA.pdf",
    size: "1.8 MB · 22 pages",
    severity: "low" as const,
    status: "complete" as const,
    risk: "3.2",
    updated: "3h ago",
  },
  {
    id: "spec-20240610-h8i9",
    contract: "Flipkart_Logistics_Contract.pdf",
    size: "4.2 MB · 56 pages",
    severity: "high" as const,
    status: "complete" as const,
    risk: "6.9",
    updated: "6h ago",
  },
  {
    id: "spec-20240609-j0k1",
    contract: "Ola_Driver_Partnership_Agreement.pdf",
    size: "2.1 MB · 31 pages",
    severity: "medium" as const,
    status: "complete" as const,
    risk: "4.5",
    updated: "1d ago",
  },
  {
    id: "spec-20240608-l2m3",
    contract: "Swiggy_Delivery_Partner_Contract.pdf",
    size: "1.9 MB · 26 pages",
    severity: "low" as const,
    status: "complete" as const,
    risk: "2.8",
    updated: "2d ago",
  },
  {
    id: "spec-20240607-n4o5",
    contract: "Zomato_Restaurant_Agreement.pdf",
    size: "2.3 MB · 34 pages",
    severity: "medium" as const,
    status: "complete" as const,
    risk: "5.1",
    updated: "3d ago",
  },
]

export default function WorkflowsPage() {
  return (
    <div className="min-h-screen">
      <Sidebar />
      <main className="ml-[248px] p-8">
        <TopBar
          title="Workflows"
          subtitle="All contract analysis workflows"
          action={<Button>New Workflow</Button>}
        />

        <GlassCard className="p-0">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[rgba(255,255,255,0.06)]">
                <th className="text-[11px] uppercase tracking-[0.08em] text-[#6B7280] font-medium text-left px-4 py-3">ID</th>
                <th className="text-[11px] uppercase tracking-[0.08em] text-[#6B7280] font-medium text-left px-4 py-3">Contract</th>
                <th className="text-[11px] uppercase tracking-[0.08em] text-[#6B7280] font-medium text-left px-4 py-3">Severity</th>
                <th className="text-[11px] uppercase tracking-[0.08em] text-[#6B7280] font-medium text-left px-4 py-3">Status</th>
                <th className="text-[11px] uppercase tracking-[0.08em] text-[#6B7280] font-medium text-left px-4 py-3">Risk Score</th>
                <th className="text-[11px] uppercase tracking-[0.08em] text-[#6B7280] font-medium text-left px-4 py-3">Updated</th>
              </tr>
            </thead>
            <tbody>
              {workflows.map((workflow) => (
                <tr
                  key={workflow.id}
                  className="border-b border-[rgba(255,255,255,0.06)] last:border-b-0 hover:bg-[rgba(255,255,255,0.02)] cursor-pointer transition-colors"
                >
                  <td className="px-4 py-4">
                    <Link href={`/workflows/${workflow.id}`} className="text-[12px] font-mono text-[#8B5CF6] hover:text-[#A78BFA]">
                      {workflow.id}
                    </Link>
                  </td>
                  <td className="px-4 py-4">
                    <p className="text-[13px] text-[#F9FAFB]">{workflow.contract}</p>
                    <p className="text-[11px] text-[#4B5563]">{workflow.size}</p>
                  </td>
                  <td className="px-4 py-4">
                    <Badge variant={workflow.severity}>{workflow.severity}</Badge>
                  </td>
                  <td className="px-4 py-4">
                    <Badge variant={workflow.status}>{workflow.status}</Badge>
                  </td>
                  <td className="px-4 py-4">
                    <span className={`text-[13px] ${workflow.risk !== "—" ? "text-[#F59E0B]" : "text-[#4B5563]"}`}>
                      {workflow.risk}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-[12px] text-[#4B5563]">{workflow.updated}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </GlassCard>
      </main>
    </div>
  )
}
